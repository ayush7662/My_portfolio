...
export default function AIChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Hi! I'm Ayush AI. Ask me anything about Ayush's skills, projects, experience, or background." }
  ])
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener('openAIChat', handleOpenAIChat)
  }, [])
  ...
  const handleSend = async (messageText?: string) => {
    ...
    try {
      ...
      const aiMessage: Message = { 
        role: "assistant", 
        content: data.message || "I apologize, but something went wrong." 
      }
          ...
          >
            ...
            <div className="bg-neon/10 border-b border-neon/20 p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-bold">Ayush AI</h3>
                  <p className="text-neon text-sm">Portfolio Assistant</p>
                </div>
              </div>
                    ...
                    {message.role === "assistant" ? (
                      <ReactMarkdown
                        components={{
                          ...
                          strong: ({ children }: any) => <strong className="text-neon">{children}</strong>,
                          code: ({ children }: any) => (
                            <code className="bg-neon/10 px-1 py-0.5 rounded text-neon">{children}</code>
                          ),
                          pre: ({ children }: any) => (
                            <pre className="bg-background/50 p-2 rounded-lg overflow-x-auto mb-2">{children}</pre>
                          )
                        }}
                      >
                        {message.content}
